...
int main(int argc, char** argv) {
  ...
  graphdyvis::astar::DemoConfig config;
  ...
  if (outputPath.empty()) {
    return graphdyvis::astar::writeDemoGraphJson(config, std::cout) ? 0 : 1;
  }
  ...
  const bool ok = graphdyvis::astar::writeDemoGraphJson(config, outputFile);
  if (!ok) {
